...
- **Top-level `JarvisLogger`** (process-level): Worker / Factory / Sampler / core summaries
  ...
  14 s", queue state).
- **Per-Sample `SampleLogger`** (detailed): one file per Sample,
  `logs/samples/{uuid}.log` (or under `SAMPLE/<uuid>/`), full trace (materialize → each
  calculator call → exception → result). Created and closed **inside the Worker**; never
  passed across processes.

The current `jarvishep/sample_logger.py` (`SampleLogger` + `BufferedSampleLogger`) and the
M1 failure-replay contract are the foundation — the new module is a wrapper around them, not
a replacement of the buffer logic. Honors the frozen `docs/specs/LOGGING_CONTRACT.md`
format. `colorlog` is an optional pretty-console dependency; `QueueHandler` keeps file sinks

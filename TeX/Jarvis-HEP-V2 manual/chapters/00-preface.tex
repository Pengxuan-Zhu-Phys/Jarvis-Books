...
every \cli{Jarvis2} subcommand, and every output file has a home chapter, and the
...
\begin{description}[style=nextline, leftmargin=1.2em]
  ...
  \item[Part~\ref{part:task-card}: The Task Card] A field-by-field reference for the
        \textsc{yaml} task card: how it is loaded, the runtime settings under
        \yamlkey{EnvReqs.V2}, the \yamlkey{Sampling} block, variables and distributions,
        the likelihood, and the shared expression language.
  \item[Part~\ref{part:backends}: Workflow Backends] How a parameter point becomes
        observables: the layered workflow model, external \yamlkey{Calculators}, file
        formats through \Portal, in-process \yamlkey{Operas}, and the \yamlkey{LibDeps}
        tool registry.
  ...
  \item[Part~\ref{part:running}: Running, Monitoring \& Outputs] The \cli{Jarvis2} command
...
\begin{terminal}
\begin{jcmd}
Jarvis2 run task.yaml
\end{jcmd}
\end{terminal}
  \item A short command mentioned mid-sentence uses \cli{Jarvis2 -{}-help} instead.

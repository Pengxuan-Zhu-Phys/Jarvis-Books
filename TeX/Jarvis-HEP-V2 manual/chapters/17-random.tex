...
\begin{keylist}
  \item[Bounds."Point number"] \req\ (\code{JV2-MTH-020} if absent); alias
  ...
  \item[Bounds.Seed] \opt, integer, default \code{0}; alias \yamlkey{seed}. The stream is
        always seeded, and \code{0} is an ordinary seed value --- so a card with no
        \yamlkey{Seed} at all still draws the same points every run. Give distinct
        nonzero seeds to runs you want to be statistically independent of each other.
\end{keylist}
...
\begin{table}[ht]
  ...
  \begin{tabular}{@{}p{0.34\linewidth}p{0.66\linewidth}@{}}
    ...
    \midrule
    Kind & fixed set \\
    Point count & \yamlkey{Bounds."Point number"} (nominal attempts) \\
    Dimensions & any \\
    Required keys & \yamlkey{Bounds."Point number"} \\
    u$\to$x mapping & yes \\

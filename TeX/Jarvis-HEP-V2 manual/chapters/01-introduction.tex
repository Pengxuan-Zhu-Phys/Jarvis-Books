...
\begin{enumerate}
  ...
  \item \textbf{Score the point.} Expressions under \yamlkey{Likelihood} combine the
        observables into a \yamlkey{LogLikelihood} value.
\end{enumerate}
...
block shown, \code{Utils}, split into \yamlkey{Operas} and \yamlkey{Likelihood}
(Table~\ref{tab:top-level-keys}).}
...
\begin{table}[ht]
  ...
  \begin{tabular}{@{}p{0.24\linewidth}p{0.20\linewidth}p{0.50\linewidth}@{}}
    ...
    \Portal & automatic   & file formats for calculator I/O --- \textsc{json},
              \textsc{csv}, \textsc{slha}, \ldots\ (Chapter~\ref{ch:io-types}) \\
...
(\cli{Jarvis2}). It installs side by side with a version~1 installation without
...
\begin{table}[ht]
  ...
  \begin{tabular}{@{}p{0.32\linewidth}p{0.62\linewidth}@{}}
    ...
    wiring in an external program & Chapters~\ref{ch:workflow-model}--\ref{ch:io-types},
      with Appendix~\ref{app:markers-tokens} open \\
    ...
    just looking something up & Chapter~\ref{ch:cli} (commands) or
      Appendix~\ref{app:yaml-index} (keys) \\
    \bottomrule
  \end{tabular}

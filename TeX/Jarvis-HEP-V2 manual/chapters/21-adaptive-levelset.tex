...
Sampling:
  ...
  AdaptiveBridson:                  # alias: adaptive_bridson
...
\begin{keylist}
  \item[target\_expression] \req. A shared-language expression
        ...
        value is $f(u)$.
  \item[target\_value] \req. The level $T$ you are tracing: $f(u) = T$.
...
plus \file{samples.\allowbreak{}csv} via \cli{Jarvis2 convert},
...
\begin{table}[ht]
  ...
  \begin{tabular}{@{}p{0.10\linewidth}p{0.14\linewidth}p{0.20\linewidth}p{0.20\linewidth}p{0.32\linewidth}@{}}
    \toprule
...
\begin{table}[ht]
  ...
  \begingroup
  \setlength{\tabcolsep}{0pt}
  \begin{tabular}{@{}p{0.21\linewidth}p{0.10\linewidth}p{0.12\linewidth}p{0.53\linewidth}@{}}
    \toprule
  ...
  \end{tabular}
  \endgroup
  \caption{\yamlkey{Sampling.AdaptiveBridson} keys. \yamlkey{contour\_precision} (an

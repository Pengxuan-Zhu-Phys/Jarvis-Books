...
\begin{table}[ht]
  ...
  \begin{tabular}{@{}p{0.24\linewidth}p{0.20\linewidth}p{0.50\linewidth}@{}}
    ...
    \yamlkey{Sampling.Seed} (alias \yamlkey{Bounds.rseed}) & \code{0} & \textsc{rng}
      seed; fix it for a reproducible run --- \yamlkey{Seed} alone is enough, you
      rarely need \yamlkey{Bounds.rseed} too \\
...
Before \cli{Jarvis2 run} or \cli{Jarvis2 check} ever touches Redis, the loaded card
...
Jarvis2 validate scan.yaml --json     # machine-readable report for tooling
\end{jcmd}
\end{terminal}
...
\cli{--strict} to \cli{Jarvis2 validate} to treat those as errors too before you
...
extra files under \file{DATABASE/}:
...
\begin{table}[ht]
  ...
  \begin{tabular}{@{}p{0.32\linewidth}p{0.62\linewidth}@{}}
    ...
    u$\to$x mapping & yes \\
    \yamlkey{Seed} role & \code{numpy} generator seed (proposal sequence only) \\

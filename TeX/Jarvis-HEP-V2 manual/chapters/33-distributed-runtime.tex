...
A running scan is the five kinds of process in Table~\ref{tab:process-family}:
...
\begin{table}[ht]
  ...
  \begin{tabular}{@{}p{0.34\linewidth}p{0.66\linewidth}@{}}
    ...
    \midrule
    \code{Jarvis:<scan>} & the control process: the Sampler, the factory that spawns
    ...
    \code{Jarvis-Worker-N} & one Sample at a time: mapping, calculators, operas,
      likelihood; plus a per-Worker file-operation helper process for
      \yamlkey{save} copies and deletions \\
    \code{Jarvis-Archiver} & batched database writes, bucket packing \\
    \bottomrule
  \end{tabular}
  \endgroup
...
small dictionaries. Products live on disk. Table~\ref{tab:redis-schema} sketches the key
shapes it uses.
...
\begin{description}[style=nextline, leftmargin=1.2em]
  ...
  \item[Layer 2 --- after the Sample.] Database rows and bucket packing belong to the
        Archiver, batched (\yamlkey{archiver.batch\_size}, flush interval) and entirely
        off the Workers' path. The handoff is \emph{direct}: a Sample's products are
        already in their final bucket location by the time the Archiver sees it, so only
        records move.
\end{description}
...
\begin{itemize}
  ...
  \item \textbf{Redis dies}: the scan cannot continue --- \Jtwo\ fails fast with a
        ...
        service and \cli{Jarvis run --resume}.

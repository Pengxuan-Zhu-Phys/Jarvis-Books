...
as \cli{Jarvis plot}, so upgrading \JPlot\ brings new figure types without a runtime
...
\JPlot\ is a core dependency of \Jtwo\ (Chapter~\ref{ch:installation}): a plain
\code{pip install Jarvis-HEP} already gives you \cli{Jarvis plot}, nothing extra to ask
for.

\begin{terminal}
\begin{jcmd}
Jarvis plot scenes/money_plot.yaml               # render a scene
Jarvis plot scenes/money_plot.yaml               # re-render after edits
\end{jcmd}
\end{terminal}
...
spelling \cli{Jarvis <scene>.yaml --plot} still routes here with a deprecation
warning.
...
\begin{terminal}
\begin{jcmd}
Jarvis run bin/scan.yaml           # produce outputs/scan-01/...
Jarvis plot scenes/overview.yaml   # scatter + LogL colormap
Jarvis plot scenes/contour.yaml    # levelset overlay
\end{jcmd}
\end{terminal}

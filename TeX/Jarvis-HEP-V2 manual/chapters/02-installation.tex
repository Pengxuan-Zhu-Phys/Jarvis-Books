...
following it, you have a working \cli{Jarvis2} command, a running Redis service, and a
...
\begin{enumerate}
  \item the \textbf{Python package} \code{jarvishep2} --- the runtime, the
        \cli{Jarvis2} command, and its two core plugin packages;
...
\begin{enumerate}
  \item Enter the \Jtwo\ source checkout:
\begin{terminal}
\begin{jcmd}
cd Jarvis-HEP-v2
\end{jcmd}
\end{terminal}
  \item Install with the \code{distributed} extra --- the recommended baseline:
\begin{terminal}
\begin{jcmd}
...
Two plugin packages install automatically as core dependencies: \Portal\ (calculator
file formats, Chapter~\ref{ch:io-types}) and \Operas\ (the operator registry,
Chapter~\ref{ch:jarvis-operas}).

Add extras to the bracket list as needed:

\begin{table}[ht]
  \footnotesize
  \begin{tabular}{@{}p{0.32\linewidth}p{0.62\linewidth}@{}}
    \toprule
    \textbf{Extra} & \textbf{Pulls in} \\
    \midrule
    \code{distributed} & \code{redis}, \code{msgpack}, \code{aiofiles} --- required
                         for real scans \\
    \code{plot}        & \JPlot\ --- enables \cli{Jarvis2 plot}
                         (Chapter~\ref{ch:jarvisplot}) \\
    \code{dev}         & \code{pytest}, \code{fakeredis}, \code{colorlog} --- the
                         test suite \\
    \bottomrule
  \end{tabular}
  \caption{Package extras. Combine freely:
  \cli{pip install -e '.[distributed,plot,dev]'}.}
  \label{tab:extras}
\end{table}

\paragraph{For developers.} To work against local checkouts of the plugin packages,
install them first --- pip then keeps your editable versions:
...
\begin{jnote}
\Jtwo\ is its own distribution (\code{jarvishep2}) with its own command
(\cli{Jarvis2}). It coexists in one environment with a version~1 installation ---
...
\begin{terminal}
\begin{jcmd}
Jarvis2 --version    # or the short form: Jarvis2 -v
\end{jcmd}
\end{terminal}
...
\centering
\JarvisVersionBanner
\caption[The \clih{Jarvis2 --version} banner.]{The \clih{Jarvis2 --version} banner,
...
\begin{jwarn}
If your shell reports \cli{Jarvis2: command not found}, the executable is not on your
...
and confirm with \cli{which Jarvis2}. Under \code{pyenv}, \cli{Jarvis2} lives in that
...
\begin{jwarn}
Start Redis \emph{before} launching a scan. If the service is down, \cli{Jarvis2 run}
...
\begin{terminal}
\begin{jcmd}
Jarvis2 portal man     # file formats (JSON, CSV, SLHA, ...)
Jarvis2 operas list    # registered operators
\end{jcmd}
\end{terminal}
...
key by key and runs it for real with \cli{Jarvis2 run}.
\end{jnote}

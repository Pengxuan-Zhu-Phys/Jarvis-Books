...
\begin{enumerate}
  \item the \textbf{Python package} \code{jarvishep2} --- the runtime, the
        \cli{Jarvis} command, and its two core plugin packages;
...
file formats, Section~\ref{sec:format-catalog}) and \Operas\ (the operator registry,
Chapter~\ref{ch:jarvis-operas}).

Add extras to the bracket list as needed; Table~\ref{tab:extras} lists what each one
pulls in:
...
\begin{table}[ht]
  ...
  \begin{tabular}{@{}p{0.34\linewidth}p{0.66\linewidth}@{}}
    ...
    \code{distributed} & \code{redis}, \code{msgpack}, \code{aiofiles} --- required
                         for real scans \\
    \code{plot}        & \JPlot\ --- enables \cli{Jarvis plot}

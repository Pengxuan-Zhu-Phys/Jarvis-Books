...
\begin{itemize}
  ...
  \item \textbf{Buckets} (default 200/dir, packed) exist precisely for network
        ...
        \yamlkey{save} files.
  \item \yamlkey{cleanup.strategy: mv\_to\_staging} buffers Sample directories through
        local scratch when final storage is remote and slow --- the move is cheap
        metadata on the same filesystem, and the Archiver does the expensive hop off
        the critical path.

...
\cli{Jarvis monitor} and \cli{Jarvis project}.
...
Sampling:
  Method: Random
  Bounds:                     # this method's own settings
    point_number: 20
    seed: 7
  Variables:
    - name: x
...
\begin{terminal}
\begin{jcmd}
Jarvis run quickstart.yaml
\end{jcmd}
\end{terminal}

\cli{Jarvis run} loads and validates the card, starts the managed processes, submits
...
\code{Jarvis}, \code{Jarvis-\allowbreak{}Worker-\allowbreak{}1},
\code{Jarvis-\allowbreak{}Worker-\allowbreak{}2},
\code{Jarvis-\allowbreak{}Archiver}, and \code{Jarvis-\allowbreak{}Redis} if it
...
\paragraph{\yamlkey{Scan}: naming the run.}
...
Same key as the first card's \yamlkey{Scan.name}, and the only one \yamlkey{Scan} takes
--- everything else about where this scan's outputs, logs, and checkpoints live follows
from it automatically (Chapter~\ref{ch:task-yaml}).
...
note that \yamlkey{Variables} and \yamlkey{LogLikelihood} did not move when the method
changed; only \yamlkey{Method} and the \yamlkey{Bounds} block under it did.
\yamlkey{radius} and \yamlkey{max\_attempt} are its two required \yamlkey{Bounds} keys, and every
...
\begin{itemize}
  \item \yamlkey{Calculators.Pools} --- global concurrency: at most \code{4} EggBox
        executions run at once, across \emph{all} Workers combined (\S\ref{sec:pools}
        in Chapter~\ref{ch:calculators}).
  ...
  \item \yamlkey{source} --- where the program template lives; \yamlkey{path} --- where
        \Jtwo\ stages a working copy of it; every command below defaults to running from
        this same directory, so neither \yamlkey{execution.path} nor a per-command
        \yamlkey{cwd} needs to repeat it.
  ...
  \item \yamlkey{execution.commands} --- the actual shell command: exactly
        \code{./eggbox.py}, as if you had typed it yourself from the pack directory
        above.
...
text, \ldots --- Section~\ref{sec:format-catalog} has every format \Portal\
supports). The
...
\begin{terminal}
\begin{jcmd}
Jarvis run bin/Example_Bridson_process.yaml
\end{jcmd}
\end{terminal}

\begin{jtip}
Do not type the card above by hand to try it: \cli{Jarvis project fetch Eggbox}
...
results: \cli{Jarvis convert <task>.yaml} re-derives \file{samples.csv} from whatever
...
\begin{itemize}
  ...
  \item To point the same skeleton at a \emph{real} external program, read the
        calculator chapters (Chapters~\ref{ch:workflow-model}
        and~\ref{ch:calculators}).
  ...
  \item To see \file{samples.csv} as a figure instead of a \code{pandas} table,
        Chapter~\ref{ch:jarvisplot} covers \cli{Jarvis plot}.
\end{itemize}

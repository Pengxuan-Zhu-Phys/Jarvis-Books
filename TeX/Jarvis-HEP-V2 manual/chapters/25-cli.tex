...
exposes one executable, \cli{Jarvis2}, and asks you to name \emph{one intent} per
...
\begin{terminal}
\begin{jcmd}
Jarvis2 <intent> <card>.yaml [flags]
\end{jcmd}
\end{terminal}

The intent comes first because it is the decision you actually make; the card is the
noun it acts on. \cli{Jarvis2 -h} prints the whole surface, and it does not print a flat
list --- the commands arrive already sorted into the five groups you actually work in:
...
\begin{table}[ht]
  ...
  \begin{tabular}{@{}p{0.24\linewidth}p{0.20\linewidth}p{0.50\linewidth}@{}}
    ...
    \bottomrule
  \end{tabular}
  \caption[The five command groups printed by \clih{Jarvis2 -h}.]{The five command
  groups printed by \clih{Jarvis2 -h}. Within the first group the order is the order you
  should use them in.}
...
Run \cli{Jarvis2 -h} when you have forgotten \emph{which} command, and
\cli{Jarvis2 <command> -h} when you have forgotten \emph{which flag} --- the help text
...
\cli{Jarvis2 validate} is byte-for-byte the file you hand to \cli{Jarvis2 run}. There is
...
(run|check|validate|convert|monitor|plot|portal|operas|project). See Jarvis2 -h.
\end{jout}
\end{terminal}
...
\begin{table}[ht]
  ...
  \begin{tabular}{@{}p{0.24\linewidth}p{0.20\linewidth}p{0.50\linewidth}@{}}
    ...
    \bottomrule
  \end{tabular}
  \caption{The three depths at which \clih{Jarvis2} will execute one card.}
...
\begin{terminal}
\begin{jcmd}
Jarvis2 validate scan.yaml
Jarvis2 validate scan.yaml --strict   # also fail on warnings
Jarvis2 validate scan.yaml --json     # machine-readable report
\end{jcmd}
\end{terminal}
...
itself --- without starting Redis, a Worker, or an Archiver. \cli{Jarvis2 run} and
\cli{Jarvis2 check} run this same gate before they do anything else, so \cli{validate}
...
\begin{terminal}
\begin{jcmd}
Jarvis2 validate bin/Example_Dynesty_Operas_Quick.yaml
\end{jcmd}
\termsep
\begin{jout}
...
\begin{terminal}
\begin{jcmd}
Jarvis2 check scan.yaml
Jarvis2 check scan.yaml --strict --console-level DEBUG
\end{jcmd}
\end{terminal}
...
\begin{terminal}
\begin{jcmd}
Jarvis2 run scan.yaml
Jarvis2 run scan.yaml --resume    # take the checkpoint, no prompt
\end{jcmd}
\end{terminal}
...
`Jarvis2 monitor R1` or `Jarvis2 ps R1` before starting another run
\end{jout}
\end{terminal}
...
\code{Jarvis2:<scan>} for the control process,
\code{Jarvis2-\allowbreak{}Worker-\allowbreak{}N:<scan>},
\code{Jarvis2-\allowbreak{}Archiver:<scan>}, \code{Jarvis2-\allowbreak{}FileOperation},
...
none of it disturbs the first. \cli{Jarvis2 monitor} reads one status snapshot.
\cli{Jarvis2 ps} lists the processes. \cli{Jarvis2 convert} exports whatever data has
...
\begin{terminal}
\begin{jcmd}
Jarvis2 monitor       # which scans are running?
Jarvis2 monitor R1    # snapshot of that one
\end{jcmd}
\end{terminal}
...
\begin{terminal}
\begin{jcmd}
Jarvis2 gen-plot-yaml scan.yaml    # writes images/<scan>/plot.yaml, prints its path
Jarvis2 plot images/<scan>/plot.yaml

# or chain them: the generator prints the path it wrote
Jarvis2 plot "$(Jarvis2 gen-plot-yaml scan.yaml)"
\end{jcmd}
\end{terminal}
...
Choose a task by its REF (for example R1):
  Jarvis2 monitor R1   view this task's live scan status
  Jarvis2 ps R1        list its control, workers, archiver, Redis
  Jarvis2 kill R1      terminate ALL processes belonging to this task
\end{jout}
\end{terminal}
...
\begin{terminal}
\begin{jcmd}
Jarvis2 kill R1              # shows the card, then asks
Jarvis2 kill R1 --yes        # no prompt (batch scripts, cleanup hooks)
Jarvis2 kill R1 --no-force   # SIGTERM only; never escalate to SIGKILL
\end{jcmd}
\end{terminal}
...
\begin{terminal}
\begin{jout}
DANGER: Jarvis2 kill R1 will terminate all 4 processes above.
...
\begin{enumerate}
  ...
  \item \textbf{Without a terminal, it refuses}. Piped or run from \cli{cron}, where
        ...
        names the flag it wants, \code{re-run with: Jarvis2 kill --yes}. Automation has
...
\begin{terminal}
\begin{jcmd}
Jarvis2 portal man             # every registered file format
Jarvis2 portal man slha        # one format's manual
Jarvis2 portal io_task.yaml    # run a standalone Portal IO YAML
Jarvis2 operas list            # every registered operator
Jarvis2 operas info math.add   # one operator's signature and doc
\end{jcmd}
\end{terminal}
...
command. \cli{Jarvis2 portal man} prints what \Portal\ can currently read and write:
...
\begin{itemize}
  \item \cli{Jarvis2 <task>.yaml} routes to \cli{run}.
  \item \cli{Jarvis2 <task>.yaml --resume} routes to \cli{run --resume}.
  \item \cli{Jarvis2 <task>.yaml --check-modules} routes to \cli{check}.
  \item \cli{Jarvis2 <task>.yaml --validate} routes to \cli{validate}.
  \item \cli{Jarvis2 <task>.yaml --convert} routes to \cli{convert}.
  \item \cli{Jarvis2 --monitor} routes to \cli{monitor} (\S\ref{sec:cli-ps-kill}).
  \item \cli{Jarvis2 <scene>.yaml --plot} routes to \cli{plot}, with a deprecation
        warning.
\end{itemize}

That list is complete, and \cli{Jarvis2 -h} prints exactly these in their own
...
\begin{table}[ht]
  ...
  \begin{tabular}{@{}p{0.32\linewidth}p{0.62\linewidth}@{}}
    ...
    \bottomrule
  \end{tabular}
  \caption{\clih{Jarvis2} exit codes --- contract-stable for scripting.}
...
\cli{Jarvis2 validate --strict}; after every edit that touches a path, a program, or an
expression, spend a minute on \cli{Jarvis2 check}; then \cli{Jarvis2 run --strict} once,
and watch it from a second terminal with \cli{Jarvis2 monitor} followed by the reference

...
outputs. When \cli{Jarvis2} loads a task card, it resolves the root in this order, and
...
appeared beside it. \cli{Jarvis2 project create} scaffolds a properly marked project
...
\textbf{Phase 1 --- static, resolved once at load time} in the control process:

\begin{keylist}
  \item[\marker{\&J/}] The project root. \marker{\&J/data/points.csv} means
        ``\file{data/points.csv} under the project root''.
  \item[\$\{Scan:name\}] The scan name; \code{\$\{Scan:root\}} (and any other suffix,
        e.g.\ \code{output}) resolves to the output root
        \file{<root>/outputs/<scan>}.
  \item[\$\{LibDeps:<name>\}] The path registered for an external tool under
        \yamlkey{LibDeps.Modules} (Chapter~\ref{ch:libdeps}); an unknown name is a hard
        error at load time.
  \item[registered executable names] A bare tool name registered under
        \yamlkey{LibDeps.registered\_executables} is replaced by its absolute path inside
        any command string.
  \item[\jtt{\textasciitilde}] Expanded to your home directory.
  \item[\code{\$\{source\}}, \code{\$\{path\}}] Only inside a module's
        \yamlkey{installation}/\yamlkey{initialization} commands: the calculator's
        \yamlkey{source} and runtime \yamlkey{path} (Chapter~\ref{ch:calculators}).
\end{keylist}

\textbf{Phase 2 --- per-Sample, resolved inside the Worker} just before execution:

\begin{keylist}
  \item[\token{@SampleID}] The Sample's \textsc{uuid}.
  \item[\token{@Sdir}] The Sample's save directory under \file{SAMPLE/}; using it forces
        the directory to be created.
  \item[\token{@PackID}] The calculator slot id the Worker holds --- the key to physical
        isolation for non-thread-safe tools (Chapter~\ref{ch:calculators}).
\end{keylist}
...
The two lists above are the complete set. Appendix~\ref{app:markers-tokens} repeats them
as one flat table if you want a print or skim copy.
...
\sampler{Bridson}'s \yamlkey{Radius} and \sampler{AdaptiveBridson}'s
...
\begin{itemize}
  ...
  \item The \cli{Jarvis2} command surface and its exit codes (Chapter~\ref{ch:cli}).

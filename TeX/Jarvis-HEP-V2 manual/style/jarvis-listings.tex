...
\lstdefinestyle{termout}{
  ...
  keywords=[2]{Jarvis,jplot,jopera},
...
\lstdefinestyle{yamlwindow}{
  ...
  aboveskip=0pt, belowskip=0pt,
  % A raw `$' (e.g. inside a `${source}' token) already typesets literally here --
  % no mathescape is set, so listings never treats it as a math shift. It only
  % LOOKS broken in editors whose syntax highlighter doesn't know `yamlwin' is a
  % verbatim listing and assumes bare `$' opens math mode. Card authors who want
  % to dodge that false positive can type the escaped form `\$' instead (the
  % ordinary LaTeX idiom most editors already recognise as literal) -- this
  % `literate' pair strips the backslash back off at typeset time so `\${path}'
  % and `${path}' render identically. Confirmed by an isolated standalone
  % compile (bare text and inside a quoted jyaml string) before landing here.
  literate=
    {±}{{$\pm$}}1
    {·}{{\textperiodcentered}}1
    {•}{{\textbullet}}1

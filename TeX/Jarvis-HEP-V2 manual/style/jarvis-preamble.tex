...
\usepackage{textcomp}      % \textperthousand, and other symbols quoted verbatim from real output
\usepackage{newunicodechar} % map the literal dot glyph U+2B24 (the banner icon) to a drawn circle
% pdfLaTeX's kernel UTF-8 table does not include U+2030 (‰) by default -- unlike
% the handful of General Punctuation characters already safe to quote verbatim
% (see jarvis-listings.tex's `literate' tables), a raw ‰ byte sequence is a hard
% "Invalid UTF-8 byte sequence" error before `literate' ever gets to substitute
% it. This registers the character so it tokenizes at all; the `literate' entry
% then still supplies the actual rendering inside jout/terminal listings.

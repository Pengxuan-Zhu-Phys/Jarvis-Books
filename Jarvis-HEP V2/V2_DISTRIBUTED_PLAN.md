...
| **D20** | 多模式 Calculator | 一个软件包在一次扫描内以 N 种用途运行（各自的 initialization / execution，必要时各自构建）；`modes` 展开为兄弟模块，运行时零新增概念。设计: [`DESIGN_CALCULATOR_MODES_2.0.md`](DESIGN_CALCULATOR_MODES_2.0.md) |
...
| D20.1–D20.4 | 多模式 Calculator（`modes`）——设计已出，待维护者拍板 §10 | D20 | — | todo |            | [`DESIGN_CALCULATOR_MODES_2.0.md`](DESIGN_CALCULATOR_MODES_2.0.md)。核心判断：**`modes` 是声明层展开，不是运行时新概念**——一个模式 = 一个继承公共声明的兄弟模块，展开后池 / pack / 层 / flowchart / selection / timeout 全部照旧，运行时零新增分支。三个曾想不清楚的问题已有答案：**(1) 池与 pack 必须每模式独立**（不是权衡：同层 calculator 在样本内并发，两个独立模式会同时跑，共用 pack 就是两个进程写同一目录，正是 clone_shadow 要防的，故不设 shared 开关）；**(2)** `required_modules` 点号寻址 `MadGraph.ProcA`，裸名 = 该模块全部模式（老卡片加 modes 后语义仍正确）；**(3)** `input/output` 按模式声明，并在装载期校验模式间 output 重名（否则后者静默覆盖前者，又是"跑得正常、结果悄悄不对"）。现有 schema 把 `modes` 定义为 `string|[string]`，表达不了需求，必须重定义（0 张卡片在用，无兼容包袱）。**主场景（维护者 2026-08-01 澄清）是「改 config 再 make」的原地重建**（老 Fortran 软件常见），证据就在自家卡片里：`iDM_Vector_Bridson.yaml` 的 `MicroOMEGAs_Vector` 与 `iDM_Axial_Bridson.yaml` 的 `MicroOMEGAs_Axial` 是同一个包、只差模型文件，却把整个模块声明复制了一份并分散在两张卡片。这类软件的**昂贵部分恰恰是模式专属的**，若把 `make` 放进模式 `installation` 会编译「模式数 × pack 数」次（2×4=8）。故 §5.4.2 增加**可选 `build` 阶段**：全局每模式构建一次（复用 LibDeps 的安装引擎与 stamp/reinstall 控制），`installation` 只拷贝产物，编译降到 2 次。三段式 `build`/`installation`/`initialization` 与现有两段式向下兼容（不写 `build` 即今天行为）。备选方案见 §5.4.3（直接用 LibDeps，零新键）。 |

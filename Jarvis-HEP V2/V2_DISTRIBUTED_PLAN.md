...
| D9.1 | Token resolution single-owner + delete sync/async twins + `CommandParser.to_picklable` | D9        | —                | **partial**                            | 2026-07-10 | **Done:** `CommandParser` sole sample-token owner (calculator fallback removed; `Sample.resolve_token` deleted); `to_picklable()` shared by core/worker_config. **Still todo:** delete calculator sync/async twins (F2). |

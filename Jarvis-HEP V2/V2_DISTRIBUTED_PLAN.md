...
| D13.8 | Configurable flat feedback return (`{uuid, logL}` default; −∞ for unusable; flat extra fields) | D13 | D13.7 | **todo** | | Design: [`DESIGN_FEEDBACK_RETURN_2.0.md`](DESIGN_FEEDBACK_RETURN_2.0.md). Wire is **flat** `{uuid, logL}` — no nested `observables`, no `status`; likelihood sets `logL=-inf` when not computed; optimizers add flat siblings via `fields` / `all_flat`; archive unchanged. |

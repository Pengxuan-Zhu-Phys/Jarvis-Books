...
Next pick: **D17.1** (complete the vocabulary — blocks strict validation, and subsumes
D13.14's Portal/AdaptiveBridson items); then **D14.1**; then **D14.1**; **D15.5** and
...
| D17.1 | Complete the closed vocabulary against the real card corpus | D17 | — | todo |            | [`DESIGN_STRICT_VALIDATION_2.0.md`](DESIGN_STRICT_VALIDATION_2.0.md) §0/§5. **Blocker for everything else in D17**: today **44 of 65** shipped `Jarvis-Examples` cards already fail validation, and the failures are schema *omissions of legitimate keys*, not bad cards — `Calculators.path` ×35 (which `worker_config.py:208` explicitly tolerates: *"Tolerate it (do not error)"* — schema and runtime contradict each other), `Modules[].deps_source` ×4, `Operas.Modules[].selection` ×1. Declare them (or demote to warning); every key in the corpus must end up declared, classified, or a justified error. |
| D17.2 | `x-jarvis-zone` classification (closed / delegated / open) + schema self-test | D17 | D17.1 | todo |            | §2. Every object block carries a zone; missing zone fails the self-test — that is what stops "open" from becoming the accidental default (as it did for `AdaptiveBridson`). Delegated I/O consults `available_io_formats()`; Portal↔manifest test relaxed to a **subset** assertion. Delegated set: Portal `type`, `Bounds.{run_nested,sampler}` (vendored dynesty API), `Operas kwargs`, `EnvReqs` V1 siblings. |
| D17.3 | `numeric` type + YAML notation quirks | D17 | D17.1 | todo |            | §3. **`1e-5` parses as a *string* in YAML** (only `1.0e-5` is a float) and physicists write it constantly; a naive `type: number` would reject cards that run fine today, since the runtime coerces with `float()`. Accept number-or-numeric-string, warn toward the canonical form, never error. Same for `on/off/yes/no` → bool where a string was meant. |
| D17.4 | Strict error semantics: one-pass report, "did you mean", exit 2, zero side effects | D17 | D17.2, D17.3 | todo |            | §4. All errors at once sorted by path (never fail-on-first — one fix pass, not N runs); `difflib.get_close_matches` against the allowed set for unknown keys; validation strictly before Redis/Worker/output-dir/log-dir creation; `run`/`check` adopt `validate`'s exit code 2. Warnings never exit. |
| D17.5 | Flip strict on + corpus regression test in CI | D17 | D17.1–D17.4 | todo |            | §5. `test_all_shipped_cards_validate_strictly` over Jarvis-Examples + `project_template` + `tests/parity_project` must be green **before** the flip lands — the gate is not a formality: a strict flip that rejects valid cards destroys trust faster than lax validation ever did. Skills + YAML_REFERENCE updated per the D16 rule. Optional one-release `JARVIS_VALIDATE_PERMISSIVE=1` escape with a removal date. |

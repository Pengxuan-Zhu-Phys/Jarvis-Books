...
| D12.1 | Calculator V1-YAML parity (string commands, `${source}/${path}`, module `selection`)    | D12       | D12.0, D12.2 (accept); D11.3 (SLHA complete) | todo                          |            | **Mainline.** Review §4.1: (1) string-list install/init/commands silently dropped → normalize to `{cmd, cwd}`; (2) `${source}/${path}` interpolate at Spec parse; (3) module-level `selection` in workflow; (4) tolerate `make_paraller`/`modes`/top-level `path` (invariant 12: never rename `make_paraller`). Accept = unmodified `Jarvis-Examples/Eggbox/bin/Example_Bridson_process.yaml` + DATABASE/sample-log golden. JSON path first; full HEP/SLHA needs D11.3. |
| D12.2 | Core logging V1 visual parity (formatter, banner, file layout)                          | D12       | D12.0            | todo                                   |            | Review §5.1: keep D0.3 queue architecture; add V1-style formatter (`·•·`/`Ϡ`, module color, `MM-DD HH:mm:ss.SSS`, raw passthrough), logo banner, `<scan>/jarvis.log` layout, per-sample log format audit. **Must land before D12.1 golden acceptance** (sample-log golden depends on it). |

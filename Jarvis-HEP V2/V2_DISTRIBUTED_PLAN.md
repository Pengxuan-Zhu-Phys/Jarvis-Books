...
| **D21.13** | **MEDIUM — 续跑把失败样本报成成功（建议 2.0.0 发版前修）** | D21 | — | todo |            | 发现于 2026-08-02 的发版前复审。**同一张卡、同一份数据，两次调用给出两个结论**：首跑 `status=partial_failure completed=20 failed=20 exit=1`；`--resume` 后 `status=success completed=40 failed=0 exit=0`，而 DATABASE 一行未变（40 行 = 20 `Failed` + 20 `Completed`）。**数据是对的**（D21.6 正常，失败点不会被无限重算），错的是**汇报**。根因：`core.py:1306` 传 `persisted_count=self._persisted_records_count`，而 `read_persisted_uuids`（`database.py:243`）收集**所有**记录的 uuid、**不看 `status`**；`reconcile_resume_ephemeral(completed=…, failed=0)` 于是把失败点全部计入 completed。两层后果：(a) 用户续跑一个崩过的扫描看到 `success / exit=0`，**可能永远不知道一半的点失败了**，包装 Jarvis2 的 CI/自动化也会当作干净；(b) `outcome.ok` 因此变 True → **续跑出了图而首跑没出图**（见 D19.2），同一份数据两种行为。**修法很小**：`reconcile_resume_ephemeral` 本来就有 `failed=` 形参，读 DATABASE 时顺带按 `status` 统计并传入即可（同一次扫描，无额外 I/O）。验收：上述场景续跑后报 `partial_failure completed=20 failed=20 exit=1`，与首跑一致。 |
| **D19.2** | **MEDIUM — 只要有一个样本失败就静默不出图**（既有问题，非 D21 引入） | D19 | — | todo |            | 发现于 2026-08-02 的发版前复审。`core.py:1165` 的 `if outcome.ok and not self._interrupt_requested:` 同时挡住 `_finalize_nested_result_csv()` 与 `_emit_plot_scenes()`，而 `partial_failure` 不属于 `ok`（`run_outcome.py:70`）。实测 20/40 失败的扫描：**只产出 flowchart，没有任何 plot YAML 与图，日志里没有一句话解释**。已核对 `git diff`：该判断与 `run_outcome.py` **均不在本轮改动内**，属长期行为。**但对物理扫描而言部分失败是常态**（SPheno / micrOMEGAs 在非物理点上失败是日常），一个 20% 失败率的扫描科学上完全有用，用户却拿不到图也不知道原因。需维护者拍板：放宽到 `partial_failure` 也出图（建议，并在图上或日志里标注失败比例），还是至少补一条明确的日志说明。归到 D19「运行结果诚实性」名下。 |
| D21.14 | 复审 LOW 项打磨 | D21 | — | todo |            | 同一轮复审的四条小问题：(1) **Bridson 续跑时打印 "X‰ samples submitted" 进度**，但那是 `advance_to_persisted_prefix` 的本地重放、一个点都没提交——恰在用户刚崩完正在恢复时显示误导信息；(2) **thread 模式** archiver 的 `ArchiveProcessor.persistence_state()` 仍是 `sorted(self.acked_uuids)`（O(N log N)），默认 process 模式已是 O(1)，影响有限但应对齐；(3) `JV2-VAR-031` 的 suggestion 文案是通用的"提供一个非空变量列表"，与实际错误（缺 `Poisson` 的 `lambda` 参数）对不上——诊断主体很好（*"unknown key(s) for type Poisson: lam; allowed: lambda, length, num"*），只是建议句没跟上；(4) **`h5clear` 是未声明的外部依赖**（来自 hdf5 命令行工具，不在 pip 的 h5py wheel 里；实测 `locking=False` 与 `r+` 均无法清除 superblock 的陈旧写标志，所以调 `h5clear` 本身是对的），应在 skill 写明或在 EnvReqs preflight 探测。另记：DATABASE/CSV 本轮新增 `sample_index` 与 `status` 两列，首个正式版可接受，但**发布说明应写明**。 |

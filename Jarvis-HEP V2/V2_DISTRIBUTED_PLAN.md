...
| **D20.5** | `JV2-MOD-005` 误杀跨块 `required_modules`——**阻塞发布** | D20 | D20.1 | todo |            | 审阅: [`D20_MULTIMODE_REVIEW_2026-08-02.md`](D20_MULTIMODE_REVIEW_2026-08-02.md) §3。实测：**7 张出厂卡片**（GMFit×2、2HDM-PT×2、HinoLLP×3）改动前 0 error、改动后 1–2 error，`raise_if_errors` → **exit 2，扫描起不来**。根因：新校验的已知名字集合只取自 `Calculators.Modules`，但 `required_modules` 合法跨块引用 **LibDeps.Modules**（`LoopTools`/`HiggsSignals`）、**Operas.Modules**（`BuildBSMPTInput`）与保留伪模块 **`Parameters`**——后者是 **V2 自己在 `flowchart.py:511/572/660` 生成的名字**。`resolve_module_layers` 对未知依赖名一向容忍（当排序提示），所以这些卡片一直正常。修法：`JV2-MOD-005` 只对**带点号且父级是已声明多模式 calculator** 的名字触发，裸名放行；或把已知集合扩成 Calculators ∪ LibDeps ∪ Operas ∪ {`Parameters`}。验收：65 张卡片 `JV2-MOD-*` 错误数 = 0，且 `Prospino.typo` 仍报错带 did-you-mean。 |
| **D20.6** | 亲和池在争用下退化成朴素 shared | D20 | D20.1 | todo |            | 审阅 §4（多线程实测，用真实 `acquire_shared_calc` + 真实 `Worker._run_shared_mode_group`）：3 模式/3 pack/3 Worker **重建率 63–67%**，正好是朴素 shared 的理论值 `1−1/M`，**亲和收益为零**；4 pack→21%、5 pack→8%、6 pack→**0%**；单 Worker 3 pack→0%（算法本身没错，是争用打垮它）；8 模式/4 pack/4 Worker→**86%**（朴素值 88%）。两条原因：(a) `acquire_shared_calc` 第一轮非阻塞扫描里目标模式一空就**立刻借用重建**，不等自己的热 pack 回来；(b) 全忙时走 `_blpop_many([目标, unassigned, 其他…])`，**BLPOP 阻塞期间是谁先被 push 谁赢**，键顺序只在调用瞬间有意义——恰恰在最需要亲和的满载时刻偏好完全失效。修法 A（实测有效）：借用前对目标模式 free list 做**有界等待**，等待 ≤3× 运行时长时 3/3/3 场景 **67%→0% 重建、墙钟 4.74s→1.85s（快 2.6×）**。修法 B（必做）：把池下限写进文档——实测规律 **`Pools[parent] ≳ 模式数 + Worker 数` 重建率归零**，而默认池取 `make_paraller`，用户极易配成"正好等于模式数"这个最差点。边界：**pack 数 < 模式数时抖动不可避免**（8 模式 4 pack 时等待策略把重建压到 58% 但墙钟反而变慢），这是 shared 的结构性代价，文档要直说。 |
| **D20.7** | `selection` 跳过的模式仍触发完整重建 | D20 | D20.1 | todo |            | 审阅 §5。调用链 `worker.py:302-321` → `Module/calculator.py:565-567`：acquire → `prepare_runtime`（**可能整套 make 重建**）→ `execute` 里才 `_should_run()`。`prepare_runtime` 全程不看 `selection`。普通 calculator 只浪费每 pack 一次安装；**shared 模式是每次取用都可能浪费一次重建**，且重建后 pack 的亲和标签被写成一个**根本没运行过的模式**，直接污染其他 Worker 的命中率。而"按参数点决定算不算某个过程"正是 multimode 最自然的搭配。修法：把 `_should_run` 提到 acquire 之前，最好在 `_run_shared_mode_group` 组 `pending` 时就滤掉。 |
| D20.8 | 文档与实现对齐 + 两处小修 | D20 | D20.5 | todo |            | 审阅 §6。(a) `Utils` 诊断措辞被回退——`cbb629f` 刚按 D18.6 去掉 *"it is not supported by V2."*，工作区又加了回来，疑似合并误带，还原即可；(b) 裸名自引用展开时含自身（`P.ns` 的 `["P"]` → `['P.ng','P.ns']`），今天无害（`resolve_module_layers` 容忍自环）但应滤掉；(c) **设计文档已与实现不符**：`DESIGN_CALCULATOR_MODES_2.0.md` 仍讲 "v1 只做 `per_mode`"、`@Mode` token、`mode_packs` 开关，实现是 **shared-only 且显式拒绝 `@Mode`**，须按实现重写并写入 §4 的池大小规律，否则下一个 agent 会照废弃 spec 干活。 |
| D20.1–D20.4 | 多模式 Calculator（`modes`）——**实现已完成，审阅见 [`D20_MULTIMODE_REVIEW_2026-08-02.md`](D20_MULTIMODE_REVIEW_2026-08-02.md)**；实现选择了 shared-only（非本设计建议的 per_mode），理由与设计 §5.6"唯一存在理由是互斥构建"一致，故认可 | D20 | — | 待 D20.5 |            | [`DESIGN_CALCULATOR_MODES_2.0.md`](DESIGN_CALCULATOR_MODES_2.0.md)。**范围（维护者定调 2026-08-01）：V2-only，不回移 V1**——V1 已是设计定型的产品，非 bug 不加新功能。这条不只是排期，它**放松了设计约束**：不同于 D18 LibDeps 与 D12.1 命令归一化（都要对齐 V1 的既有行为与老写法），D20 **没有可对齐的参照物**（V1 的 `analyze_config_multi()` 方法体是 `pass`），也**没有兼容包袱**（0 张卡片在用且不会有），故 `build` 阶段 / `mode_packs` / `@Mode` token 都可按 V2 自己的最优解定形。三条执行推论：(a) 不动 V1 的桩（设计未完，非 bug）；(b) `modes` 只写进 V2 的 YAML_REFERENCE 与 skill，V1 手册不提；(c) **多模式卡片不可回跑 V1**——实测 V1 会无声造出空壳模块（`execution`/`installation`/`basepath`/`type` 全缺），到运行期才抛与病因无关的 `AttributeError`，此事实须写进 skill。注意区分：放松的只是"对 V1 的兼容"，**不是"对 V2 自身的一致"**——模式级 `installation` 仍应接受纯字符串命令列表，理由是 V2 模块级就是这个形态，而非 V1 parity。核心判断：**`modes` 是声明层展开，不是运行时新概念**——一个模式 = 一个继承公共声明的兄弟模块，展开后池 / pack / 层 / flowchart / selection / timeout 全部照旧，运行时零新增分支。三个曾想不清楚的问题已有答案：**(1) 池与 pack 必须每模式独立**（不是权衡：同层 calculator 在样本内并发，两个独立模式会同时跑，共用 pack 就是两个进程写同一目录，正是 clone_shadow 要防的，故不设 shared 开关）；**(2)** `required_modules` 点号寻址 `MadGraph.ProcA`，裸名 = 该模块全部模式（老卡片加 modes 后语义仍正确）；**(3)** `input/output` 按模式声明，并在装载期校验模式间 output 重名（否则后者静默覆盖前者，又是"跑得正常、结果悄悄不对"）。现有 schema 把 `modes` 定义为 `string|[string]`，表达不了需求，必须重定义（0 张卡片在用，无兼容包袱）。**§6.1 已被 §5.5 更正**（维护者以 Prospino 为例）：并发论据只排除「所有模式共用同一 pack 目录」，排除不了「共用一个池、取用时把 pack 重建成所需模式」。Prospino 改 config 重新 make，**重建快、过程多**，用 per_mode 会造 12×4=48 份拷贝，`shared` 池才对；MadGraph 反之。故最终为 **`mode_packs: per_mode | shared`**，**默认 per_mode**（误配代价不对称：per_mode 用错只多占一次性磁盘，shared 用错是全程持续重建）。`shared` 实现要点：stamp 记 `mode`；**重建前先删 stamp、成功后再写**（否则中途崩溃会留下 stamp 说 A 而目录是半个 B，下次取用 A 会跳过重建、直接跑坏产物）；并须校验 `Pools[<Module>] >= 单样本内并发模式数`，否则每个样本都会 acquire 超时失败（`JV2-MOD-006`）。**§5.6 收窄适用范围（维护者观察）**：micrOMEGAs 的 vector/axial 是同一包的**并列子目录、可共存**，声明两个普通模块即可，**不需要 modes**；MadGraph 同理。**multimode 的唯一存在理由是「多种用法在一份安装里互斥」**（Prospino 的 `final_state_in` 是同一构建目标的编译期配置）。此判据须写进 YAML_REFERENCE 与 skill 开头，否则用户会滥用 modes。收窄后取舍重算：朴素 `shared` 的命中率约 `1/模式数`，声明 4 个过程时**约 75% 的取用触发重建**，"重建快"不足以让它成立；建议 **v1 只实现 `per_mode`**（零重建、行为可预测），待磁盘成为真实问题后再加 §5.6.3 的**模式亲和池**（每模式一个 free list + 借用，稳态下每个 pack 沉淀到一个模式，磁盘 ∝ pack 数而非模式数 × pack 数）。 **另一主场景是改 config 再 make 的原地重建**（老 Fortran 软件常见），证据就在自家卡片里：`iDM_Vector_Bridson.yaml` 的 `MicroOMEGAs_Vector` 与 `iDM_Axial_Bridson.yaml` 的 `MicroOMEGAs_Axial` 是同一个包、只差模型文件，却把整个模块声明复制了一份并分散在两张卡片。这类软件的**昂贵部分恰恰是模式专属的**，若把 `make` 放进模式 `installation` 会编译「模式数 × pack 数」次（2×4=8）。故 §5.4.2 增加**可选 `build` 阶段**：全局每模式构建一次（复用 LibDeps 的安装引擎与 stamp/reinstall 控制），`installation` 只拷贝产物，编译降到 2 次。三段式 `build`/`installation`/`initialization` 与现有两段式向下兼容（不写 `build` 即今天行为）。备选方案见 §5.4.3（直接用 LibDeps，零新键）。 |

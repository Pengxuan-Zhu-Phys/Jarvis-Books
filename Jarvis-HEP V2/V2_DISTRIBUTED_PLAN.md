...
| D9.2 | `Distributor` registry + IO format registry (unknown type errors)                     | D9        | —                | **partial** (IO done)                  | 2026-07-10 | **IO half done:** `jarvishep2/io_portal.py` bridges to Jarvis-Portal (`create_entry_point_registry`); calculator has no `type==JSON` hardcode; unknown types raise `UnsupportedIOTypeError`; design [`DESIGN_PORTAL_IO_2.0.md`](DESIGN_PORTAL_IO_2.0.md); tests `tests/test_io_portal.py`. **Still todo:** Distributor sampler registry (F7). |

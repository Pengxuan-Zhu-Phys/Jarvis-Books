...
| — | `Calculators.Modules[].modes` — **待设计**（不是死代码） | D18 | — | 设计未定 |            | 维护者澄清（2026-08-01）：`modes` 表达“同一个软件包的多种用法”——一次扫描内调用两次、但**初始化与执行命令不同、可能各需独立构建**。V1 只设了标志没做完，**不是**做完后废弃；YAML 形态尚未定案。开工前至少要回答：多模式与 PackID 槽位的关系（每模式一套 pack 还是共享）、`required_modules` 如何引用某个模式、`execution.input/output` 是否按模式分别声明。**先不立实现 WP**，此行仅为登记，避免再被误判为死代码。 |

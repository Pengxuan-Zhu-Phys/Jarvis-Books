...
Last updated: 2026-07-31. Branch `jarvis2`; D17.1–D17.8 complete. **V1 migration status + open gaps: [`V1_MIGRATION_STATUS_2026-07-31.md`](V1_MIGRATION_STATUS_2026-07-31.md)** (12 V1 samplers unmigrated; `--refs`/`--benchmark` absent; **D17.9** root-zone hole). **D18**: LibDeps install-once missing ([`DESIGN_LIBDEPS_2.0.md`](DESIGN_LIBDEPS_2.0.md)); 2026-08-01 second-pass adds **D18.6** (`Utils.interpolations_1D` migrated to Operas but users are told only "remove it" — 15 cards) and **D18.7** (5 V1 distributions hard-rejected). D13.11 landed; suite last recorded 608 passed / 8 pre-existing failures — triage tracked in D13.15.
...
| D18.1 | LibDeps token completeness (`${LibDeps:path}`, `${LibDeps:make_paraller}`, `@{ROOT path}`) | D18 | — | todo |            | [`DESIGN_LIBDEPS_2.0.md`](DESIGN_LIBDEPS_2.0.md) §3.3. Today only `${LibDeps:<Module>}` resolves; **`${LibDeps:path}` raises `KeyError` and is used 36× in shipped cards**, `${LibDeps:make_paraller}` (for `make -j…`) likewise. Generalize `_replace_libdeps_token` to module names **plus** the block's scalar keys. `@{ROOT path}` has no token although the value is already computed at preflight (`environment_requirements.py:142`, `summary["ROOT path"]`) — wire it, or fail naming `EnvReqs.CERN_ROOT`. |
| D18.2 | Declare `LibDeps` in the task-card schema (zone `closed`) | D18 | — | todo |            | §3.5. `path`, `make_paraller`, `Modules[]` (`name`/`required_modules`/`installed`/`installation.{path,source,commands}`), `registered_executables`. Accept and ignore V1's `installed:` flag — the stamp file is authoritative. **Precondition for D17.9**: the root is `delegated` (so top-level typos vanish silently) largely because LibDeps was never declared. |
| D18.3 | LibDeps install engine (control-process preflight) | D18 | D18.1, D18.2 | todo |            | §3.2/§3.3. **Nothing in V2 executes LibDeps commands today** — `library.py` is 41 lines of symlink+registry, vs V1's 159-line `Module/library.py`. Normalize V1's plain-string `commands` (same as D12.1 did for calculators); order by `required_modules` via `workflow.resolve_module_layers`; concurrency capped by `make_paraller`; runs **after** env preflight (builds may need ROOT) and **before** Redis/Workers, so a failed build stops the scan before anything spawns. Per-module log `logs/<scan>/library-<name>.log`. |
| D18.4 | LibDeps install control + `--skip-library-installation` | D18 | D18.3 | todo |            | §3.1. Reuse D13.11's `jarvis_install.json` + `.jarvis_install_stamp.json` verbatim (one mechanism for operators to learn); the epoch fan-out is unnecessary here (one copy, one writer) but keep the field for symmetry. **Do not port V1's interactive `Reinstall? (y/n)` prompt** — the control process must stay non-interactive (nohup / scheduler / future cluster submitter); the flag expresses the same intent ahead of time. |
| D18.5 | LibDeps docs + skill | D18 | D18.3 | todo |            | YAML_REFERENCE LibDeps section; new `skills/shared-libraries.md` with a verified card (D16 rule). Document the first-run build cost and that editing source files inside a library tree does **not** trigger a rebuild — same deliberate limitation as calculators. |

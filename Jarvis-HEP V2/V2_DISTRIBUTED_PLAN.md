...
| D13.8 | Configurable feedback return (minimal uuid+LogL default; fields/all extension) | D13 | D13.7 | **todo** | | Design: [`DESIGN_FEEDBACK_RETURN_2.0.md`](DESIGN_FEEDBACK_RETURN_2.0.md). Project observables on Worker before `publish_feedback`; stamp `worker_config["feedback_return"]` from sampler/YAML; Dynesty/MCMC minimal; ALS fields(target symbols); archive path unchanged. |

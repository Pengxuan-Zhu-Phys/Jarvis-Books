...
| D17.6 | Strict-validation polish: plural-typo hint, actionable numeric errors, hermetic corpus gate | D17 | D17.1–D17.5 | todo |            | Design §7.1 (none blocking). **(1)** `task_schema.py:117` matches only `'x' was unexpected`, but jsonschema emits `'a', 'b' were unexpected` for 2+ keys — so with **two** typos the suggestion degrades to the literal placeholder `'the unexpected key'` and the "did you mean" is lost: the user gets less help exactly when they made more mistakes. Match both forms, `get_close_matches` per key. **(2)** the §3 numeric union made errors generic — `'abc'` now reports *"is not valid under any of the given schemas"* with a boilerplate suggestion and no example; special-case it to *"expected a number (e.g. `0.05` or `1.0e-5`), got the string `'abc'`"*. **(3)** `test_task_schema_corpus.py:14` globs a **sibling** `Jarvis-Examples` checkout and asserts `>=65` with no skip guard, so on a clean CI runner the gate fails for the wrong reason and reads as a schema regression — vendor a corpus snapshot under `tests/fixtures/` or `skipUnless` + a job that checks out both repos. A gate that cannot run everywhere is not yet a gate. Minor: the 27-key `Allowed keys:` dump is redundant once "did you mean" names the fix. |

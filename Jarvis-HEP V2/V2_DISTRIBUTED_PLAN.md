...
| D13.11 | Review fixes @2daf417: install fingerprint, feedback-drop log, slot-release atomicity, atomic CSV write | D13 | — | todo |            | [`CODE_REVIEW_2026-07-23.md`](CODE_REVIEW_2026-07-23.md) §1. **(1) HIGH — no operator control over calculator reinstall**: install reuse skips `installation` on a fingerprint match, and the fingerprint (by design) does not see source-file edits — so after editing their calculator the user silently gets results from stale code, with `JARVIS_FORCE_CALC_INSTALL=1` (undocumented, all-or-nothing) as the only lever. Fix per [`DESIGN_CALC_INSTALL_CONTROL_2.0.md`](DESIGN_CALC_INSTALL_CONTROL_2.0.md): a `jarvis_install.json` in the **calculator folder** (outside PackID dirs) recording per-pack install state and carrying a `reinstall` flag; a monotone `reinstall_epoch` bumped once by the control process makes a single flip reinstall **all** packs exactly once (control process is the sole writer of that file, Workers only write their own pack stamp — race-free without locking); reuse/reinstall becomes one line in the control-process run header. **Do not** add content hashing of the source tree — explicitly rejected by the maintainer. (2) port the D13.7b unmatched-`hep:feedback` warning into `FeedbackSampler.wait_for_generation` — it only landed in `RedisEvaluationPool`, so every MCMC/ensemble/PT/AdaptiveBridson barrier still drops stray records silently (§1.2). (3) `Worker._force_release_pack` pops the pack before releasing and swallows failures → slot leaks past the watchdog sweep; keep until confirmed, log, and make release atomic via Lua (§1.3). (4) `convert_hdf5_to_csv` + `samples.csv` exporters write in place → Ctrl-C leaves a truncated CSV that skip-if-exists makes permanent; use tmp+`os.replace` like `write_install_stamp` already does (§1.4). (5) rename adaptive `timeout` → `generation_timeout` + document the per-generation semantic (§1.5). |

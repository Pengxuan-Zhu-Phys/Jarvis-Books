...
| D13.8 | Configurable feedback return (minimal uuid+LogL default; no status; fields/all extension) | D13 | D13.7 | **todo** | | Design: [`DESIGN_FEEDBACK_RETURN_2.0.md`](DESIGN_FEEDBACK_RETURN_2.0.md). Wire `{uuid, observables}` only — **no sample status** on feedback (failure = missing LogL/fields); project on Worker; stamp `worker_config["feedback_return"]`; Dynesty/MCMC minimal; ALS fields; archive keeps full status. |

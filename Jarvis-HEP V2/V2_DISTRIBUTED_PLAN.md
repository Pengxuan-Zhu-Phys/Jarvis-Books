...
| D20.1–D20.4 | 多模式 Calculator（`modes`）——设计已出，待维护者拍板 §10 | D20 | — | todo |            | [`DESIGN_CALCULATOR_MODES_2.0.md`](DESIGN_CALCULATOR_MODES_2.0.md)。核心判断：**`modes` 是声明层展开，不是运行时新概念**——一个模式 = 一个继承公共声明的兄弟模块，展开后池 / pack / 层 / flowchart / selection / timeout 全部照旧，运行时零新增分支。三个曾想不清楚的问题已有答案：**(1) 池与 pack 必须每模式独立**（不是权衡：同层 calculator 在样本内并发，两个独立模式会同时跑，共用 pack 就是两个进程写同一目录，正是 clone_shadow 要防的，故不设 shared 开关）；**(2)** `required_modules` 点号寻址 `MadGraph.ProcA`，裸名 = 该模块全部模式（老卡片加 modes 后语义仍正确）；**(3)** `input/output` 按模式声明，并在装载期校验模式间 output 重名（否则后者静默覆盖前者，又是"跑得正常、结果悄悄不对"）。现有 schema 把 `modes` 定义为 `string|[string]`，表达不了需求，必须重定义（0 张卡片在用，无兼容包袱）。边界：公共且昂贵的构建应放 **LibDeps**（D18 已实现），因为模块级 `installation` 会在每个模式的每个 pack 里各跑一次。 |

...
Lua slot release, atomic CSV). New **D13.14**: the task-card schema plumbing is sound but
the per-method schemas are empty — no method constrains `Bounds`, so misspelled/mistyped
sampler keys pass validation and silently fall back to defaults.
...
Next pick: **D13.14** (schema `Bounds` vocabulary — silent misconfiguration); then **D14.1**; **D15.5** and
...
| D13.14 | Task-card schema: give per-method schemas their real `Bounds` vocabulary + fail-closed on unschema'd methods | D13 | — | todo |            | [`SCHEMA_REVIEW_2026-07-31.md`](SCHEMA_REVIEW_2026-07-31.md). The `schema/` plumbing is sound (local-only registry, `check_schema` at load, actionable diagnostics, Portal↔manifest test-guarded) but **0 of 17 method schemas constrain `Bounds`**, and 14 of 17 contain nothing but a *tautological* `Method: const <own name>` (the dispatcher already selected the file by that name) plus a shared `required: [Variables]`. **(1) HIGH** — verified: `DRAM Bounds:{chainz:4, stepz:100}` (every key misspelled) and `Bounds:{chains:"四"}` (string for an int) both pass validation clean; the run then silently falls back to defaults, so the user asks for 4 chains and gets 1. Transcribe the per-method key vocabulary from YAML_REFERENCE §6.11–6.13 into each method schema (`properties` + types + `additionalProperties: false`); **MCMC family first** — nested methods already have partial cover from the legacy contracts layer (`Dynesty Bounds:{walkz:9}` → `JV2-BND-001`), the MCMC family has none, which also makes coverage unpredictable between families. **(2)** unknown *I/O format* fails closed (`JV2-SCH-002`) but a **registered sampler with no manifest entry fails open** — simulated: a fake registered method with garbage `Bounds` produced zero diagnostics. Mirror the IO behavior and add the missing **Distributor↔manifest** test (Portal↔manifest has one; manifest↔directory has none either — all four boundaries are clean today but three are hand-held). **(3)** `test_manifest_gives_each_sampling_method_its_own_schema` asserts only URI uniqueness — it passes trivially while all contents are equivalent; strengthen to require method-specific content. **(4)** root `$id` `…/task-card.json` vs file `task-card-v2.schema.json` (only file where `$id` ≠ path). |

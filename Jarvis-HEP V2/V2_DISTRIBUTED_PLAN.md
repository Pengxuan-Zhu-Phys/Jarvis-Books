...
| D22.3 | `Sampling.Mapper` 扁平 map schema + `JV2-MAP-0xx` 装载期诊断 | D22 | D22.2 | **done** | 2026-08-04 | 设计 §4/§5。**YAML（`251db09` 后的最终形态）：`Sampling.Mapper` 是 `[{name, expression}]` 列表**——本行原写的扁平 `{名字: 表达式}` 已被 `251db09 refactor(d22)` 改掉，自由对象形式现在被 `JV2-MAP-001`/schema **拒绝**（实测），`YAML_REFERENCE §7.2` 正文与 `contracts/mapper.py:188` 均已对齐，但 §7.2 的**小节标题仍写着 "flat name → expression"**，属残留措辞。**无嵌套 `derive`。** `Sampling` closed zone 一键增量。封闭性：`free_symbols ⊆ Variables ∪ Mapper 名 ∪ 常量`。DAG 拓扑 + 环检测、命名冲突、CSV 拒绝、`JV2-MAP-050/051` 先验告警。 |
...
### D23 — Jarvis-Operas 命名空间常量（`pdg.mZ` 无括号形式）
...
**已实现并复审（2026-08-05）**：JO `384f116`（含 `build_constant_dicts` / `build_sympy_dicts(constants=)` / `_constants.constant_decl` / `pdg` 12 个常量 / `ConstantValue`+`ConstantNamespace`）+ HEP2 `9dba8b3`（闸门放宽 + 接线 + `JV2-OPR-002`）。
命名最终采用 **`mZ` 风格**（非设计文档初稿的 `m_Z`）；本节内所有 `m_Z` 字样以代码为准读作 `mZ`。
**复审实测**：设计 §10 的 A1–A9/A11 全过（见各行 Notes）；JO `pytest -q` **179 passed**，HEP2 相关 4 个文件 **111 passed + 22 subtests**。
**复审新开 5 个 WP：D23.8–D23.12**，其中 **D23.9（HIGH）与 D23.8（MEDIUM）建议 2.0.0 发版前修**。
**第二轮复审（2026-08-05，工作树未提交）：D23.8–D23.12 五项修复逐条实测通过** ——
D23.8 指纹 12 条、match/drift/legacy/无 integrity 四路分支全对；
D23.9 Mapper 里常量/函数/混合/纯表达式四类全跑通（`t=0` 时 `mz == pdg.mZ` 逐位相等），真卡片 `Jarvis2 validate` 绿灯（含 `selection: "abs(mz) < pdg.mZ"` 引用 Mapper 派生名 + 常量），**pipeline 构造仅 0.8 ms**（全局注册表已热，没有出现担心的每次 ~1 s 冷启）；
D23.10 pickle/deepcopy 双双恢复且往返后 `__getattr__` 仍给建议；D23.11 `pdg.mz → mZ`、`pdg.ALPHAEM → alphaEM`、`pdg.zzz` 正确无建议；D23.12 `SIGNATURE` 已换成 `VALUE`/`UNIT`/`SOURCE`。
回归：JO `pytest -q` **181 passed**（+2 新增 `test_constants.py`），HEP2 5 个文件 **131 passed + 22 subtests**（含 `test_distributed_resume.py`），设计 §10 的 A1–A9 全部维持原值，V1 侧仍是 11 个 `SimpleNamespace` / `UndefinedFunction` / 差集 12 条。
**第二轮新开：D23.13（MEDIUM，D23.8 的副作用）与 D23.14（LOW，既有缺陷）。**

| ID | 工作包 | 主题 | 依赖 | 状态 | 完成日 | 备注 |
|----|--------|------|------|------|--------|------|
| **D23.1** | **JO：`build_constant_dicts` + `build_sympy_dicts(constants=)`**（仓库：Jarvis-Operas） | D23 | — | **done** | 2026-08-05 | **复审实测确认**（JO `384f116`）：`core/spec.py`、`core/registry.py` 果然一行未改；`build_sympy_dicts` 保持二元组返回 + `mapping` 第一位置参数 + `constants` 关键字默认 `None`；`_refresh_sympy_dicts` 已显式传 `constants=build_constant_dicts()`（注释也点明了 mapping 非 None 会走 V1 路径这个坑）。A2 实测 `sqrt(pdg.mZ**2 + pdg.mW**2) * x` 的 lambdify 体为 `return 121.555092541448*x`（代数折叠）；A3 实测 12 个常量往返**逐位相等**。 以下为原始工作包描述。 设计 §6。约 40 行，**唯一的架构级改动**。常量 = arity-0 `OperaFunction` + `flags={"constant"}` + `metadata["value"]`，**`core/spec.py` 与 `core/registry.py` 一行不改**（实测 flags/metadata 已能穿透注册表；`registry.call('pdg.m_Z')` → 91.1876；同名重复注册被 `OperatorConflict` 挡住）。硬契约 C-1/C-2：**返回值必须保持二元组、`mapping` 必须保持第一位置参数、新参数必须关键字且有默认值**——V1 `jarvishep/inner_func.py:119-127`/`:160-168`、PLOT `jarvisplot/inner_func.py:178-186`、HEP2 `operas_functions.py:137` 四处都按 `func_locals, numeric_funcs = build_sympy_dicts(mapping, include_all=True)` 解包，且 **V1 那两处外面包着 `except Exception: pass`——破了不报错，只静默丢掉整个 Operas 函数表**。合并规则：常量优先、跳过同名 arity-0 impl（这是正常路径不是错误路径，`build_register_dicts` 必然含它），**不要 raise**。`_refresh_sympy_dicts`（`integration.py:80-88`）同步。 |
| D23.2 | JO：`pdg` namespace 首批常量 + `constant_decl` helper（仓库：Jarvis-Operas） | D23 | D23.1 | **done** | 2026-08-05 | **复审实测确认**：注册 12 个常量 `pdg.{mZ,mW,mt,mh,me,mmu,mtau,alphaEM,alphaSMZ,GF,hbarc,c}`，`source: PDG 2024` / `unit` / `since` 全部填齐；`is_protected_namespace('pdg') == True`；`jopera list --namespace pdg` 与 `jopera info pdg.mZ` 均显示 value+unit。**命名定为 `mZ` 风格**（设计初稿写的是 `m_Z`）。 以下为原始工作包描述。 设计 §7。`namespaces/_constants.py` 的 `constant_decl` **由 `value` 生成 `numpy_impl`**，杜绝值漂移。首批只放高频无歧义量（`m_Z/m_W/m_t/m_h/m_e/m_mu/m_tau/alpha_em/alpha_s_mZ/G_F/hbarc/c`），**不要**塞混合角、CKM 元素这类依赖参数化约定的量。metadata `source`（PDG 版本）**必填**。`catalog/index.py:13-19` 注册 + `namespace_policy.py:10-21` 把 `pdg` 加进保护名单（常量表是物理事实，不能被用户 op 覆盖）。**单位只做文档不做换算**；用 `sp.Float` 不用 `sp.Rational`（实测 `Rational(1,3)*x` 在 `x=3` 处返回恰好 1.0，语义不同）。 |
| **D23.3** | **HEP2：放宽发现闸门 + 接线常量**（`jarvishep2/operas_functions.py`） | D23 | D23.1 | **done** | 2026-08-05 | **复审实测确认**（`9dba8b3`）：采用了带守卫的正则，A8 门禁 **14/14**——4 条真阳性（含裸常量 `pdg.hbarc`）全中，`/usr/local/bin/foo.sh`、`./run.sh`、`results/out.dat`、`C:\\tools\\a.exe` 全挡，`0.5`/`1.5e3`/`x*1.0`/`sin(t)*2.5`/`Pi`/`x_1 . y` 全不中。A9 实测无限定名卡片 `triggered=False`。改名按 "reference" 口径完成且**保留 4 个 pre-D23 别名**（`expression_uses_operas_function` 等仍在 `__all__`）；`build_constant_dicts` 的 import 包了 try/ImportError，对旧版 Operas 降级为不折叠。 以下为原始工作包描述。 设计 §8。`:39` 的 `_QUALIFIED_FUNCTION_CALL` 要求限定名后必须跟 `(`，所以 `pdg.m_Z` 根本不触发发现。**不要只删尾部 `\s*\(`**——实测裸放宽会误命中 `/usr/local/bin/foo.sh`、`./run.sh`、`results/out.dat`、`C:\tools\a.exe`；用带守卫的 `(?<![/\\.\w])[A-Za-z_]\w*(?:\.[A-Za-z_]\w*)+(?![\w.]*[/\\])`，实测这四条全挡住且真阳性一条不丢。**误判代价要报实数**：一次 Operas 冷启发现实测 **0.86–1.19 s/进程**，Worker 是 spawn 的，N 个 Worker 就是 N 次。`:136-137` 改传 `constants=build_constant_dicts(target_registry)`。**改名**：`_QUALIFIED_FUNCTION_CALL` / `expression_uses_operas_function` / `operas_expression_functions_required` 全串改成 "reference" 口径（常量不是 function call）；`expression_uses_operas_function` 是 `__all__` 导出（`:184`），留别名或同步全部调用点。 |
| D23.4 | HEP2：`Operas.Modules[].operator` 指向常量的校验诊断 + `YAML_REFERENCE` 表达式语法补"限定常量" | D23 | D23.3 | **done** | 2026-08-05 | **复审实测确认**：真卡片 `Jarvis2 validate` 报 `JV2-OPR-002 Operas.Modules[0].operator 'pdg.mZ' is a Jarvis-Operas constant, not a module operator.`，message/suggestion/example 三段齐全；`YAML_REFERENCE_2.0.md:1362-1365` 已写入无括号语法与"`pdg.mZ()` 是错误"，`:1378` 的 `operator` 行已标注 `JV2-OPR-002`。校验器在 Operas 不可用或注册表异常时返回空集合（既不 fail-open 也不 fail-closed）。 以下为原始工作包描述。 设计 §8.4/§11.5。`operator: pdg.m_Z` + `call_mode: call` 运行时能跑通并返回 91.1876，但语义无意义；给一条诊断指向"直接在 `expression` 里写 `pdg.m_Z`"。 |
| **D23.5** | **V1 / PLOT 回归门禁**（跨仓） | D23 | D23.1 | **done** | 2026-08-05 | **复审实测确认（本轮亲跑）**：V1 `jarvishep/inner_func.py` 未改动；`build_expression_context()` 与 `update_funcs(None)` 均跑通、**未触发 `except Exception: pass`**；V1 侧 11 个 namespace 对象**全部是 `SimpleNamespace`**（无一个 `ConstantNamespace`），`pdg.mZ` 在 V1 仍是 `UndefinedFunction`（= 今天新增任意 Operas 函数的既有行为）。V2 的 `numeric_funcs` ⊂ V1 的，差集正好 **12 条**（被折叠的常量 impl），与设计 §5 预期逐条吻合。"V1 多出一批可调用零参函数"这一点按设计 §5 判定为**非 V1 功能变更**，维护者未提出异议。 以下为原始工作包描述。 设计 §10 A5/A6，**本 WP 是发布闸门**。冻结 V1 `jarvishep/inner_func.py` 不动，比对 `build_expression_context()` 返回的 `parse_locals`/`numeric_modules` 键集合与改动前**逐键相同**；并**临时把 V1 的 `except Exception: pass` 改成 `raise` 跑一遍**，确认没有被触发。PLOT `inner_func.py:178-186` 同法。V1 定位说明（待维护者确认）：本设计下 V1 会多出一批**可调用的零参函数**，这与"给 Operas 新增 namespace，V1 自动看见"是同一类事，**不是 V1 功能变更**；若要连这个也挡，需让 `build_register_dicts` 跳过 `constant` flag，代价是 `jopera call pdg.m_Z` 一并失效——**建议不挡**。 |
| D23.6 | JO 文档：playbook §6.5「新增常量」+ `docs/architecture.md` 公共面 + 目录生成器分开渲染常量 | D23 | D23.2 | todo | | **复审实测：仍未做，且现在可以做了（D23.1/D23.2 已落地）。** 缺口逐条：(a) `MAINTAINER_NAMESPACE_FUNCTION_PLAYBOOK_CN.md` 两份副本**均无 "constant" 字样**——维护者今天无法照手册加一个新常量（设计 §10 A10 未过）；(b) `docs/architecture.md` 与 `README.md` **均未出现 `build_constant_dicts`**，它没进 "Stable Public Surface"；(c) `docs/functions/` 目录生成器未分开渲染常量。**HEP2 侧的文档已完成**（`YAML_REFERENCE_2.0.md:1362-1365/1378`，记在 D23.4）。手册需补一句：常量的 `defs/` 可为空——这是对 playbook §4 目录规范的唯一偏离。 以下为原始工作包描述。 设计 §11。**实现完成前不要先写**（否则又是一份记录未实现行为的文档，重蹈 D22.1 覆辙）。playbook 需补一条：常量的 `defs/` 可为空——这是对 §4 目录规范的唯一偏离。 |
| D23.7 | LOW（可选）：`ConstantValue` / `ConstantNamespace` 可读报错 | D23 | D23.1 | **done** | 2026-08-05 | **已采纳并实测**：`pdg.mZ()` → `TypeError: 'pdg.mZ' is a Jarvis-Operas constant, not a function; write 'pdg.mZ' without parentheses.`；`pdg.m_Z` → `AttributeError: 'pdg.m_Z' is not registered in Jarvis-Operas. Did you mean mZ? Known: …`。`build_sympy_dicts` 只给**含常量的 namespace** 换类型，纯函数 namespace 保持 `SimpleNamespace`（实测 V1 侧 11 个全是 `SimpleNamespace`）。**遗留两个缺陷见 D23.10（pickle 回归）与 D23.11（大小写建议）。** 以下为原始工作包描述。 设计 §7.4。现状报错实测很差：`pdg.m_Z()` → `TypeError: 'Float' object is not callable`；`pdg.m_H` → `AttributeError: 'types.SimpleNamespace' object has no attribute 'm_H'`。两个小类即可修好，**原型已实测**（代数折叠不受影响、可 pickle、参与运算后退化为普通 `sp.Float`）：`pdg.mZ` → "Did you mean m_Z? Known: hbarc, m_W, m_Z"。采用时 `integration.py:74-75` 只给**含常量的 namespace** 换类型，纯函数 namespace 保持 `SimpleNamespace`。 |
| **D23.8** | **MEDIUM — 常量值不进 checkpoint 指纹：改 Operas 再 `--resume` 会静默换物理** | D23 | D23.1 | **done** | 2026-08-05 | **建议 2.0.0 发版前修。** 实测 `runtime_checkpoint.py:168-175` 的 `integrity` 只有 `config_hash`（整张归一化卡片）+ `variable_signature` + `mapper_hash`（`mapper_block_fingerprint` = Variables + Mapper 文本，见 `mapper.py:360-382`）。**常量值不在卡片里，它来自 Operas 注册表**——升级 Operas 版本或改 `overrides.json` 后，同一张卡的 `selection` 接受集会变，而 `selection` 排在 accepted_index/uuid 分配**之前**（`bridson.py:195-206`），resume 又靠 `advance_to_persisted_prefix` 重放 `propose_next()` ⇒ **`accepted_index` 错位、uuid 与坐标脱钩**。这与 D22.5 挡的是同一类事故，D22.5 只覆盖了卡片文本一侧。改法：把 `build_constant_dicts()` 的稳定摘要（名字→值，排序后 `stable_json_hash`）并进 `integrity`（建议独立字段 `operas_constants_hash`，便于给出"是 Operas 变了不是卡片变了"的精确报错），`--resume` 时不一致即硬拒。严格说这个隐患对 Operas **函数**也成立（换实现同理），但常量把发生概率拉高一个量级——更新 PDG 数值是常规操作，重写 `math.add` 不是。 |
| **D23.9** | **HIGH — `Sampling.Mapper` 里用不了任何 Operas 引用（常量与函数皆然）** | D23 | — | **done** | 2026-08-05 | **建议 2.0.0 发版前修。** 实测真卡片 `Mapper: {mz: "pdg.mZ * cos(t)"}` → `Jarvis2 validate` 报 `JV2-MAP-007 cannot compile Mapper expression for 'mz': 'Symbol' object has no attribute 'mZ'`，**suggestion 是 "Fix the sympy expression syntax for this Mapper entry" —— 建议错误**：语法没问题，是命名空间根本不在。两个叠加根因：(1) `mapper.py:399` `MapperPipeline.__init__` 硬写 `self._expression_context = ExpressionContext()`，**永远拿不到 Operas namespace**；(2) 就算修了 (1)，闸门也看不见——`_EXPRESSION_TEXT_KEYS` 只认 `expression`/`selection`/`target_expression`，而 `Sampling.Mapper` 的键是**用户自取的名字**（`x`/`mz`），嵌套遍历按 `_as_expression_text=False` 走，实测 Mapper-only 卡片 `operas_expression_references_required` 返回 **False**。**这不是 D23 引入的**——实测 `Mapper: {x: "math.add(t, 1.0)"}` 同样炸，是 D22 留下的洞；但 D23 让 `pdg.mZ * cos(t)` 成了最自然的写法，所以现在升级为 HIGH。改法：(a) `MapperPipeline` 接受注入的 `ExpressionContext`（Worker 与控制侧都已各自持有 Operas 感知的 context），(b) 闸门遍历把 `Sampling.Mapper` 的**值**按表达式文本处理（加一条 mapping-key 特例，或让 `Mapper` 整块以 `_as_expression_text=True` 递归），(c) `JV2-MAP-007` 在命中 `'Symbol' object has no attribute` 时改出指向 Operas 发现的 suggestion。验收：`Mapper: {x: "pdg.mZ * cos(t)"}` 的卡片 validate 通过并跑出正确 `x`。 |
| D23.10 | MEDIUM — `ConstantNamespace` 破坏了 pickle/deepcopy（既有可拷贝对象的回归） | D23 | D23.7 | **done** | 2026-08-05 | 实测：`pickle.dumps/loads` 与 `copy.deepcopy` 对 `parse_locals["pdg"]` **双双失败**——`TypeError: ConstantNamespace.__init__() missing 1 required positional argument: '_jo_namespace'`；对照组 `parse_locals["math"]`（普通 `SimpleNamespace`）正常。根因：`integration.py` 给 `ConstantValue` 补了 `__reduce__`，但 `ConstantNamespace` 没补，而它的 `__init__` 有一个必填位置参数，`SimpleNamespace` 的默认 reduce 协议重建不出来。**目前无活跃触发点**（实测 `factory.py:417/421/644` deepcopy 的是 config dict，不是 parse_locals；Worker 在 spawn 后各自重建 context），所以定 MEDIUM 不定 HIGH——但这是一个原本可拷贝对象的回归，是颗地雷。改法：给 `ConstantNamespace` 加 `__reduce__`（与 `ConstantValue` 同法），并补一条单测断言 `parse_locals` 每个 namespace 对象都能 pickle 往返。 |
| D23.11 | LOW — 大小写笔误拿不到正确建议（`pdg.mz` 是最可能的笔误） | D23 | D23.7 | **done** | 2026-08-05 | 实测 `pdg.mz` → `Did you mean mt, mh, me?`（**没有 `mZ`**）；`pdg.MZ` → `Did you mean mZ?`（对）；`pdg.hbar` → `Did you mean hbarc?`（对）。根因：`integration.py` 的 `get_close_matches(name, known, cutoff=0.5)` 区分大小写，`ratio("mz","mZ") = 0.4 < 0.5`。而 `mZ`/`alphaSMZ` 这套命名下**最可能的笔误恰好是大小写**（维护者本人在讨论中写的就是 `pdg.mz`）。改法：先做 casefold 匹配再回落到 difflib，或对 `known` 建一张 casefold→原名的索引优先命中。 |
| D23.12 | LOW — `jopera info` 把 lambda 默认参数技巧泄给用户 | D23 | D23.2 | **done** | 2026-08-05 | 实测 `jopera info pdg.mZ` 的 `SIGNATURE` 显示 `(_v=91.1876)`——那是 `constant_decl` 里 `lambda _v=value: _v` 的实现细节。常量本就不该显示函数签名；建议 `flags` 含 `constant` 时把 `SIGNATURE` 行换成 `VALUE` / `UNIT` / `SOURCE` 三行。 |
| **D23.13** | **MEDIUM — Operas 临时不可用会删掉 checkpoint 并退化成整跑（D23.8 引入）** | D23 | D23.8 | **done** | 2026-08-05 | **实测复现**：把 `operas_constants_fingerprint()` 换成返回 `{}`（**这正是它 `ImportError` / `except Exception` 两个分支的返回值**），`check_operas_constants_fingerprint` 立刻判 `False`，`core.py:1431-1441` 打日志 `Jarvis-Operas constant values changed since the checkpoint was written…` 并 **`os.remove(path)` 删除 checkpoint**，随后 `_resume_policy = "fresh"` ⇒ `_load_persisted_database_state` 不执行（`core.py:509`）、`_reset_redis_for_fresh_run()` 执行（`:528`）、`advance_to_persisted_prefix` 不执行（`:536-538`）⇒ **整个扫描从 index 0 重来**，而旧 HDF5 行还在盘上。后果不是丢数据（HDF5 未动），是**白跑一遍全部物理 + checkpoint 已删无法回头 + 报错内容与真实原因不符**（常量根本没变，只是 Operas 那一刻导不进来 / 注册表抛异常）。对比：`check_mapper_fingerprint` 不会误判，因为它哈希的是卡片本身，永远可得；D23.8 是**第一条依赖外部包状态**的 resume 门。改法：`operas_constants_fingerprint()` 在不可用时返回 **`None`**（与"合法的空常量表 `{}`"区分开），`check_operas_constants_fingerprint` 遇到 `None` **跳过并 WARNING**，只在两边都拿得到且确实不同时才拒绝；拒绝时是否仍要删 checkpoint 也值得复议。验收：模拟 `ImportError` 时 `--resume` 正常续跑并留下一条 WARNING。 |
| D23.14 | LOW（既有缺陷，非 D23 引入）— 诊断文本里的非 ASCII 字符被转义成 `\uXXXX` 打给用户 | D23 | — | **done** | 2026-08-05 | 实测 `Jarvis2 validate` 输出 `suggestion: … (pdg.mZ, math.add, …), check that…`。根因：`task_validation.py:28-32` `_ascii_diagnostic()` 的 docstring 写的是"Escape **user-provided** non-ASCII text"，但 `format_line`（`:47-61`）把它**无差别地套在 code/path/message/hint/suggestion/example 六个字段上**，于是**我们自己写的**诊断文案也被转义。**不是 D23 回归**——全仓已有约 17 条作者文案命中：`contracts/methods.py:171,182,218`、`contracts/operational.py:103,115,176,307,358,369,380`、`contracts/variables.py:313`、`contracts/nested.py:59,60,94`、`contracts/mapper.py:130,188` 里的 `≥` / `→` / `—` 全部渲染成 `≥` / `→` / `—`。两条改法二选一：(a) 只对回显的用户值做转义，作者文案原样输出；(b) 保持 ASCII-only 约定，把这 17 处改成 `>=` / `->` / `...` 并加一条测试禁止新增非 ASCII 文案。 |

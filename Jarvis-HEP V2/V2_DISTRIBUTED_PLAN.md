...
Last updated: 2026-07-31. Branch `jarvis2` @ `43b1122` (schema subsystem). D13.11 landed; suite last recorded 608 passed / 8 pre-existing failures — triage tracked in D13.15.
...
Next pick: **D14.1** (D17.1–D17.5 done 2026-07-31; D17.6 is optional polish); then **D14.1**; **D15.5** and
...
> D12.0–D12.8 (2026-07-14…16); D13.1–D13.5b (2026-07-16/17); D17.1–D17.5 (2026-07-31); D13.6–D13.10
...
| D17.6 | Strict-validation polish: CJK table alignment, plural-typo hint, actionable numeric errors, hermetic corpus gate | D17 | D17.1–D17.5 | todo |            | Design §7.1 + the `0cb3379` summary-table review. **Priority raised**: the new multi-error summary table is genuinely useful and appears in both `validate` and `run` (exit 2 preserved), but it now displays the older defects side by side — the table correctly lists *both* misspelled keys while the suggestion directly beneath still reads `Remove or rename 'the unexpected key'`. **(1) NEW — CJK breaks the table.** `_format_issue_summary_table` pads with `f"{v:<{w}}"`, which counts *characters*, while CJK glyphs occupy **two** terminal columns; `_ellipsize` truncates by characters too. Verified: a card with a Chinese key (`暗物质质量`) makes that row **5 columns wider** than the borders; a fully-CJK message truncated at 72 chars would overflow by **~72 columns** (half a screen). This project's users write Chinese and messages embed user key names verbatim. Fix: compute display width via `unicodedata.east_asian_width` ('W'/'F' → 2) for both padding **and** ellipsizing. **(2)** `task_schema.py:117` matches only `'x' was unexpected`; jsonschema emits `'a', 'b' were unexpected` for 2+ keys, so the suggestion degrades to the literal placeholder and the "did you mean" is lost — the user gets less help exactly when they made more mistakes. **(3)** the §3 numeric union made type errors generic (`'abc' is not valid under any of the given schemas` + boilerplate + no example); special-case to *"expected a number (e.g. `0.05` or `1.0e-5`), got the string `'abc'`"*. **(4)** `test_task_schema_corpus.py:14` globs a **sibling** `Jarvis-Examples` checkout and asserts `>=65` with no skip guard — on a clean runner the gate fails for the wrong reason (no CI exists yet, so this is latent). **(5)** minor: `_format_issue_summary_table([])` raises `TypeError` from `max(int)` — unreachable behind the `>=2` guard, but the helper is module-level; use `max(..., default=…)`. Minor: the 27-key `Allowed keys:` dump is redundant once "did you mean" names the fix. |

| D17.7 | ASCII-only task cards (`JV2-ENC-001`): non-ASCII key or string value → error + exit | D17 | — | todo |            | Maintainer decision 2026-07-31 (design §8): *YAML 中明确不支持中文，若有中文直接退出*. **Zero corpus impact — ships immediately**: all 78 shipped cards (65 Examples + 7 template + 6 parity) are already pure ASCII, so unlike D17's strictness this needs no migration phase. **Scope**: keys **and** string values; **comments stay unrestricted** (the parser discards them — verified), and the hint on every violation should say so, turning a refusal into a one-line fix. Today a Chinese **key** errors only as a confusing *"additional properties"*, and a Chinese **`Scan.name` passes** — then becomes a directory name that propagates into paths, tar members and HDF5 attributes. **Ban all non-ASCII, not CJK specifically**: a Cyrillic `а` inside `nаme:` is visually identical to ASCII and would otherwise slip through to a baffling "unknown key" against a list that appears to contain it — one ASCII rule kills that whole class, with no "is this Chinese enough" ambiguity. **No field is exempt** (maintainer: *"scan.name 也不允许用中文，英文是唯一 YAML 语言"*) — including free-text `description`; `Scan.name` is the sharpest case since it becomes a directory name. **Runs before schema validation** so the message is honest; exit 2, nothing started. **Supersedes D17.6 item (1)**: escape non-ASCII in diagnostics (`\uXXXX`) so every printed line is ASCII by construction — then the summary table's character count equals its display width and the `east_asian_width` plumbing can be dropped rather than written. |

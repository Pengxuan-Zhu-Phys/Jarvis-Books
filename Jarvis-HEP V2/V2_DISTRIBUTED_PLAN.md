...
**D13 review (2026-07-17)**: D13.1–D13.5b done and archived; **D13.6 was
mis-marked done** with an uncommitted diagnostics tail — downgraded to
in-progress; **D13.7** (review fixes) added. See
[`D13_SAMPLERS_REVIEW_2026-07-17.md`](D13_SAMPLERS_REVIEW_2026-07-17.md).
Next pick: **D13.6** (commit the tail) → **D13.7** (fail-loud pool dispatch +
feedback-drop logging + nuisance re-run default + MCMC ESS), then **D14.1**
(cluster worker template — no hard dependency on D13, may run in parallel).
...
| D13.6 | D13 acceptance + docs closure (YAML_REFERENCE §6 methods, samplers_catalog, DATABASE chain columns) | D13 | D13.2–D13.5 | **in-progress** (was miscarded `done`) | 2026-07-17 | YAML §6.10–6.13 + catalog hierarchy committed. **`diagnostics_export.py`, `tests/test_d13_acceptance.py`, and the `core.py`/`dynesty_sampler.py`/`mcmc_sampler.py` wiring that calls it are still uncommitted** — reviewed in place and passing (8/12 subtests), but a WP is not `done` until its diff lands (see [`D13_SAMPLERS_REVIEW_2026-07-17.md`](D13_SAMPLERS_REVIEW_2026-07-17.md) §0). Commit the tail, then re-mark done. |
| D13.7 | D13 review fixes: fail-loud pool dispatch, feedback drop logging, nuisance re-run default, MCMC ESS | D13 | D13.6 | todo |            | [`D13_SAMPLERS_REVIEW_2026-07-17.md`](D13_SAMPLERS_REVIEW_2026-07-17.md) §2: (a) `RedisEvaluationPool.map` must raise, not silently execute locally, on a dispatch-signal miss (§2.1); (b) log dropped/unmatched `hep:feedback` records instead of discarding silently (§2.2); (c) resolve `Module/profile1d.py` `re_run_physics` default against V1's actual `nuisance_LogLikelihood.py` behavior, document the key in YAML_REFERENCE (§2.3 — needs a V1-source/maintainer decision, not just code); (d) add per-chain ESS to `chain_history.csv`/`sampler_summary.json` (§2.4, closes design goal 6). |

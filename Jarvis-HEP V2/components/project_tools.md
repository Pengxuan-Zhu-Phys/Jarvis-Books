...
> **As-built reality:** **none of `project_scaffold.py` / `project_packager.py` /
> `official_project_library.py` exist under `jarvishep2/`**, and there are **no `Jarvis2 project …`
> subcommands** ([cli.md](cli.md) ships only `task_yaml` + `--monitor`/`--check-modules`/`--resume`
> + Redis flags). The design below is retained for history.

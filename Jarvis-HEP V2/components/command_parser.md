...
(`${LibDeps:…}`), `_SCAN_PATTERN` (`${Scan:…}`), `_registered_name_pattern(name)`.
...
| `from_config(config, *, project_root=None, task_root=None)` (`@classmethod`) | build from YAML: project root, scan name, LibDeps paths, register executables. |
...
| `resolve_static(text)` | **Phase 1**: `expand_j` (`&J/`), `${Scan:…}`, `${LibDeps:…}`, registered names, `~`; raise on any unresolved static token. |
...
| `_replace_scan_token` / `_replace_libdeps_token` / `_replace_registered_names` | regex substitution helpers. |
...
| `_build_libdeps_paths(config, *, project_root)` | map LibDeps module names → resolved paths. |

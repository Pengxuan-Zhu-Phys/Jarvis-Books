...
reused from V1.
**Status**: ⚠️ **NOT PORTED TO V2** (as-built @ `jarvis2` `d0de31a`).

> **As-built reality:** **none of `utils.py` / `versioning.py` / `dataconvert.py` / `plot.py` /
> `observable_io.py` exist under `jarvishep2/`.** The small helpers that *did* ship live in
> purpose-specific modules instead: two-column log formatting in [`log_kv.py`](logger.md), KV/JSON
> helpers in [`database.py`](datarecorder.md) / [`io_json.py`](io_system.md), and the built-in
> `LogGauss` math in [`inner_func.py`](expression.md). There is **no HDF5→CSV `--convert`, no
> plotting, and no version banner**. The design below is retained for history.
